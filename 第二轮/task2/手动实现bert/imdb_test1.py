...
def text_padding(encoded_reviews, seq_length):
    ...
    return reviews
token_list = text_padding(token_list, onelen)
...
def make_data():
    ...
    while positive != batch_size / 2 or negative != batch_size / 2:
        ...
        # 弄一个注意力，表示inputs中需要注意的序号。（无cls和sep）
        cand_maked_pos = [i for i, token in enumerate(input_ids)
                          if token != word2idx['[CLS]'] and token != word2idx['[SEP]']]  # candidate masked position
        #  0.8概率生成mask遮住单词，0.1替换成其他错的单词
        ...
        # Zero Padding (100% - 15%) tokens  # 预测用的也需要统一到最大，多余就填充
        if max_pred > n_pred:
...
input_ids, segment_ids, masked_tokens, masked_pos, isNext = \
    torch.tensor(input_ids, dtype=torch.long), torch.tensor(segment_ids, dtype=torch.long), torch.tensor(masked_tokens,
                                                                                                         dtype=torch.long), \
...
def gelu(x):
    """
      ...
      0.5 * x * (1 + torch.tanh(math.sqrt(task1 / math.pi) * (x + 0.044715 * torch.pow(x, 3))))
...
class MultiHeadAttention(nn.Module):
    def __init__(self):
        ...
        self.linear0 = nn.Linear(n_heads * d_v, d_model)

    def forward(self, Q, K, V, attn_mask):
        # q: [batch_size, seq_len, d_model], k: [batch_size, seq_len, d_model], v: [batch_size, seq_len, d_model]
        residual, batch_size = Q, Q.size(0)  # 获得残差连接和batchsize
        # (B, S, D) -proj-> (B, S, D) -split-> (B, S, H, W) -trans-> (B, H, S, W)
        # view：维度重构     transpose：调换1维和2维（类似转置）
